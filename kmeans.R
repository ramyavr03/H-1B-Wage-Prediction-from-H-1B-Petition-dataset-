...
write.csv(training_set,"mydata.csv")
write.csv(test_set,"mydataTest.csv")

datasetTrain2 = read.csv('mydata.csv',header=TRUE)
...
classifier = naiveBayes(x = dsTrain1[-5],y = dsTrain1$prevailing_wage1)
test_pred1 <-predict(classifier,newdata =dsTrain1[-5])
...
classifier2 = naiveBayes(x = dsTrain2[-5],y = dsTrain2$prevailing_wage1)
test_pred2 <-predict(classifier2,newdata =dsTrain2[-5])
